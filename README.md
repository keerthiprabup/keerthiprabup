<!-- <div align="center">

# Tr4c3 - Keerthi Prabu P

**Multi-disciplinary Developer | Cybersecurity & Red Team | Quantum Computing | Full-Stack Engineer**

![Profile Views](https://komarev.com/ghpvc/?username=keerthiprabup&style=flat-square&color=blue)
![GitHub followers](https://img.shields.io/github/followers/keerthiprabup?style=flat-square)
![GitHub User's stars](https://img.shields.io/github/stars/keerthiprabup?style=flat-square)

</div> -->

<h1 align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=32&duration=2000&pause=1000&color=00FF41&width=600&lines=Keerthi+Prabu+P;Red+Teamer;Quantum+Computing+Enthusiast;Full-Stack+Developer" alt="Typing SVG" />
</h1>

<div align="center">
  
  [![GitHub followers](https://img.shields.io/github/followers/keerthiprabup?label=Followers&style=social)](https://github.com/keerthiprabup)
  [![Twitter Follow](https://img.shields.io/twitter/follow/keerthiprabup?label=Twitter&style=social)](https://twitter.com/keerthiprabup)
  
</div>
---


## About Me

I'm a passionate developer and red team professional with expertise spanning **cybersecurity, quantum computing, full-stack development, and embedded systems**. Currently leading pentesting initiatives at **team bi0s**, India's premier cybersecurity team.

As an ELC student at **Amrita School of Engineering, Amritapuri**, I combine theoretical knowledge with practical expertise in:
- 🔒 **Penetration Testing & Red Team Operations** - Leading team bi0s pentest division
- 🔬 **Quantum Computing** - Algorithm and quantum circuit design
- 💻 **Full-Stack Development** - Modern web applications with TypeScript, React, and Next.js
- 🌐 **Cloud & Network Security** - Azure AD, cloud infrastructure, network tools
- 🤖 **Embedded Systems & IoT** - Fire detection, smart metering systems
- ⚡ **Competitive Programming** - Algorithm optimization and problem-solving

---

## 🛠️ Technical Expertise

### Languages & Frameworks
```
Python • JavaScript • TypeScript • C • C++ • PHP • Java
React • Next.js • Express • Flask
```

### Security & Cloud
```
Penetration Testing • Network Analysis • Red Team Operations
Azure AD • AWS • Cloud Security • CTF Challenges
```

### Technologies
```
Quantum Computing (Qiskit) • Blockchain • IoT • Embedded Systems
Git • Docker • SQL • RESTful APIs
```

---

## 🚀 Featured Projects

### Security & Penetration Testing
| Project | Description | Status |
|---------|-------------|--------|
| **team bi0s** | Leading India's premier cybersecurity team | Active |
| **Penetration Testing Framework** | Comprehensive red team tooling | Active |
| **Network Tools** | Custom network analysis utilities | Live |
| **CTF Challenges** | Natas, Flaws, LordofSqli walkthroughs | Complete |

### Quantum Computing
| Project | Description | Tech Stack |
|---------|-------------|-----------|
| **Qwordle** | Quantum Wordle solver using Grover's Algorithm | Python, Qiskit |
| **Quantum-Bench** | Quantum backend performance benchmarking | Python |
| **Alt-F4-Nexus24** | Quantum computing research & experiments | Jupyter |

### Full-Stack Applications
| Project | Description | Tech Stack |
|---------|-------------|-----------|
| **bi0s.in** | Official team bi0s website | TypeScript, Next.js, Tailwind |
| **RideShare** | Ride-sharing platform with real-time features | JavaScript, Python, Flask |
| **Pentest Recruitment Portal** | Recruitment system for pentesters | HTML, CSS, JavaScript |
| **Hunter** | Competitive programming platform | JavaScript, Node.js |

### Embedded Systems & IoT
| Project | Description | Tech Stack |
|---------|-------------|-----------|
| **Embedded Fire Detection** | Real-time fire detection system | Python, Computer Vision |
| **Smart Meter on Blockchain** | Decentralized metering system | JavaScript, Blockchain |
| **Automatic Pill Dispensor** | IoT-enabled medication system | PHP, Embedded C |

### Research & Innovation
| Project | Description | Focus |
|---------|-------------|-------|
| **Mindmap-AD** | Active Directory attack/defense mindmaps | Security Research |
| **AzureAD-Attack-Defense** | Comprehensive Azure AD security guide | Cloud Security |
| **PayloadsAllTheThings** | Web security payloads collection | Offensive Security |
| **Drowsiness Detection** | Real-time driver monitoring system | ML/Computer Vision |

---

## 📊 Expertise Areas

### 🔐 Cybersecurity
- Red Team Operations & Penetration Testing
- Network Security & Analysis
- Cloud Security (Azure AD, AWS)
- CTF Competitions & Security Research
- Web Application Security

### 🔬 Quantum Computing
- Grover's Algorithm Implementation
- Quantum Circuit Design
- Quantum Simulation & Benchmarking
- Hybrid Classical-Quantum Systems

### 💻 Software Development
- Full-Stack Web Applications
- TypeScript & Modern JavaScript
- React & Next.js Frameworks
- Competitive Programming
- Algorithm Optimization

### ⚙️ Systems & Hardware
- Embedded Systems Development
- IoT Solutions
- Blockchain Integration
- Real-time System Design
- Computer Vision Applications

---

## 🎓 Education

**Electrical and Computer Engineering**  
Amrita School of Engineering, Amritapuri (2024 - Pursuing)

**Achievements:**
- Pull Shark Achievement on GitHub
- Active contributor to security research
- CTF challenge curator and walkthrough creator

---

## 🔗 Connect With Me

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github)](https://github.com/keerthiprabup)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/_tr4c3)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/keerthiprabup)
[![Facebook](https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/tr4c3.734/)
[![Discord](https://img.shields.io/badge/Discord-7289DA?style=for-the-badge&logo=discord&logoColor=white)](https://discordapp.com/users/1033217966373863544)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:keerthiprabup@gmail.com)

</div>

---

## 🏆 Notable Highlights

- **Red Team Lead** at India's premier cybersecurity team (bi0s)
- **Quantum Computing Innovator** - Practical implementations of quantum algorithms
- **Full-Stack Developer** - Built production applications with modern tech stack
- **Security Researcher** - Active in penetration testing and vulnerability research
- **Open Source Contributor** - Maintains and contributes to security tools
- **Competitive Programmer** - Algorithm problem-solving across multiple platforms

---

## 🎯 Current Focus

- 🔴 Leading pentesting operations and red team initiatives at team bi0s
- 🌩️ Quantum computing research and algorithm implementation
- 🛡️ Cloud security architecture and Azure AD security
- 📡 IoT and embedded systems innovation
- 💡 Cross-domain projects bridging security, quantum, and full-stack development

---


<div align="center">

**Let's collaborate on innovative security solutions, quantum computing projects, and cutting-edge full-stack applications!**

</div>


